...
class TestWarrior:
    ...
    def blade_dancer(self):
        return BladeDancer(character_lvl=83, enemy_lvl=83)

    def test_calculate_attributes(self, blade_dancer):
        expected_attributes = {"strength": 219, "agility": 198, "intellect": 3}
        assert blade_dancer.character_attributes_lvl == expected_attributes

    def test_calculate_strength(self, blade_dancer):
        assert blade_dancer.character_attributes_lvl["strength"] == 219

    def test_calculate_agility(self, blade_dancer):
        assert blade_dancer.character_attributes_lvl["agility"] == 198
