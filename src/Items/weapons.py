...
class Weapons(Item, ABC):
    ...
    def calculate_slow(self):
        if ("frost" in self.damage_type or "frost" in self.damage_type) and "+" in self.damage_type:
            return self.slow_factor[self.damage_type.split("+", 1)[1]][0] * self.lvl
        elif ("frost" in self.damage_type or "frost" in self.damage_type) and "+" not in self.damage_type:
